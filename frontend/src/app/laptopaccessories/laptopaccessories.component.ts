...
@Component({
  selector: 'app-laptopaccessories',
  templateUrl: './laptopaccessories.component.html',
  styleUrls: ['./laptopaccessories.component.css']
})
export class LaptopaccessoriesComponent implements OnInit {
laptopbags = [];
...
opendialog(i)
{
   this.dialog.open(DialogLaptopaccessoriesComponent, {data: i});
}
// tslint:disable-next-line: typedef
opendialog1(i){
  this.dialog.open(DialogMousesComponent, {data: i});
}
 // tslint:disable-next-line: typedef
  // tslint:disable-next-line: typedef
  getOrders()
...
ngOnInit(): void {
  this.getOrders();
  this.auth.getlaptopbags()
  .subscribe(
    res => {
      this.laptopbags = res;
      console.log(this.laptopbags);
    },
    err => {
